...
export function registerVaultTools(server: McpServer, deps: ToolDeps): void {
  server.registerTool('vault-create', {
    description: 'Create a Dominion vault config with named access tiers. Signs and publishes the config as a kind 30481 vault-config event. Returns the created event, publish result, and tier names.',
    inputSchema: {
