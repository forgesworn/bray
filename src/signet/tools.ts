...
export function registerSignetTools(server: McpServer, deps: ToolDeps): void {
  server.registerTool('signet-badge', {
    ...
    inputSchema: {
      pubkey: z.string().describe('Hex pubkey of the user to assess'),
    },
    annotations: { title: 'Signet Badge', readOnlyHint: true, openWorldHint: true },
  }, async (args) => {
    const result = await handleSignetBadge(deps.trust!, args)
  ...
  server.registerTool('signet-vouch', {
    ...
    inputSchema: {
      pubkey: z.string().describe('Hex pubkey of the user to vouch for'),
    ...
    const result = await handleSignetVouch(deps.ctx, deps.pool, args)
    return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }] }
  })
  ...
  server.registerTool('signet-credentials', {
    ...
    inputSchema: {
      pubkey: z.string().describe('Hex pubkey of the user whose credentials to fetch'),
    },
    annotations: { title: 'Signet Credentials', readOnlyHint: true, openWorldHint: true },
  }, async (args) => {
    const result = await handleSignetCredentials(deps.pool, deps.ctx.activeNpub, args)
    return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }] }
  })
  ...
  server.registerTool('signet-policy-check', {
    ...
    inputSchema: {
      pubkey: z.string().describe('Hex pubkey of the user to check'),
    ...
    const result = await handleSignetPolicyCheck(deps.pool, deps.trust!, deps.ctx.activeNpub, args)
    return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }] }
  })
  ...
  server.registerTool('signet-challenge', {
    ...
    inputSchema: {
      verifierPubkey: z.string().describe('Hex pubkey of the verifier being challenged'),
    ...
    const result = await handleSignetChallenge(deps.ctx, deps.pool, args)
    return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }] }
  })
}
