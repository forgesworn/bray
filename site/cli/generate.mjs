...
const groups = [
  ...
  },
  {
    ...
    rows: [
      ...
      { ask: 'What is this group about?', cli: 'npx nostr-bray group-info <relay-url> <group-id>' },
      { ask: 'Show me the last twenty group messages', cli: 'npx nostr-bray group-chat <relay-url> <group-id> --limit 20' },
      { ask: 'Tell the group I will be late', cli: 'npx nostr-bray group-send <relay-url> <group-id> "running late"' },
      { ask: 'Inspect the members, admins and roles', cli: 'npx nostr-bray group-inspect <relay-url> <group-id>' },
      { ask: 'Open a closed group called Ships Log', cli: 'npx nostr-bray group-create <relay-url> ships-log --name "Ships Log" --closed' },
      { ask: 'Rename the group and open it up', cli: 'npx nostr-bray group-update <relay-url> <group-id> --name "Open Log" --open' },
      { ask: 'Make them an admin of the group', cli: 'npx nostr-bray group-add-user <relay-url> <group-id> <pubkey-hex> --role admin' },
      { ask: 'Remove them from the group', cli: 'npx nostr-bray group-remove-user <relay-url> <group-id> <pubkey-hex>' },
      { ask: 'Read the roles supported by the relay', cli: 'npx nostr-bray group-roles <relay-url> <group-id>' },
    ],
  },
  {
