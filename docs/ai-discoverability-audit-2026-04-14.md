...
## Overall Score: 7.8 / 10 (up from 6.0)
...
| Trust | GitHub presence | 3.5 | 3.5 | Unchanged (Tier 4) |
...
- **Quality:** ~7.7/10 (weighted across Comms + DevEx + AI Collab + MCP)
- **Visibility:** ~2/10 (0 GitHub stars, only Glama listing, not in MCP registry)
- **Gap:** significantly under-visible
- **Interpretation:** product substance now clearly outpaces reach. Internal doc work has hit diminishing returns; the bottleneck is external distribution. Tier 4 (registry submissions, GitHub metadata, security policies enabled) is the critical next step.
...
### Tier 4 -- distribution (biggest Test C lift, ~3-5 points)
1. Rewrite GitHub repo description -- remove em dash (OPSEC-grade style rule), match package.json trust-aware positioning
2. Add 8 GitHub topics: `trust`, `web-of-trust`, `signet`, `dominion`, `nip-46`, `attestations`, `bunker`, `ai`
3. Enable secret scanning + Dependabot security alerts (one-click)
4. Upload 1280×640 social preview image
5. Disable (or populate) the empty wiki
6. **Submit to `registry.modelcontextprotocol.io`** -- zero Nostr servers there; first-mover window is open
7. File awesome-mcp-servers PR (submission doc already drafted at `docs/mcp-aggregator-submission.md`)
8. File awesome-nostr PR (drafted at `docs/awesome-nostr-pr.md`)
9. Submit to Context7 (LLM-facing docs index -- currently no Nostr content indexed there)
10. Publish a launch post on forgesworn.dev ("Why we built bray"), link from README
